...
int main() {
	...
	for (int i = 0; i < SCENE_PAYLOAD_SIZE; i++) {
		...
		if (actual != expected) {
			std::cout << "Test failed at i = " << i << ", expected " << expected << " but got " << actual << std::endl;
		}
	}
...
int main() {
	...
	std::cout << "DMA took " << (after_dma - before_dma) << " times units" << std::endl;
	...
	for (int h = 0; h < image_height; ++h) {
	    for (int w = 0; w < image_width; ++w) {
	        auto pixel_center = scene.pixel_00_loc + (w * scene.pixel_delta_u) + (h * scene.pixel_delta_v);
	...
	std::cout << "SW took " << (after_sw - before_sw) << " times units. after: " << after_sw << " before: " << before_sw  << std::endl;
